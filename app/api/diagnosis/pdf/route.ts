...
function diagnosisFromRow(row: Database["public"]["Tables"]["diagnoses"]["Row"]) {
  const interpretations = Array.isArray(row.possible_interpretations)
    ? row.possible_interpretations
    : [];
...
export async function GET(request: Request) {
  try {
    ...
    if (userError || !user) return Response.json({ error: "Your session is no longer valid." }, { status: 401 });

    const diagnosisId = new URL(request.url).searchParams.get("id")?.trim();
    if (!diagnosisId) return Response.json({ error: "Diagnosis id is required." }, { status: 400 });
    ...
    const { data: diagnosis, error } = await supabase
      ...
      .eq("id", diagnosisId)
      .single();
    if (error || !diagnosis) return Response.json({ error: "Diagnosis not found." }, { status: 404 });
    if (diagnosis.status !== "final" || !diagnosis.reviewed_at || !diagnosis.finalised_at) {
      return Response.json({ error: "Parent PDF download is available only after human review and school approval." }, { status: 409 });
    }

    const [workspaceResult, studentResult, assessmentResult] = await Promise.all([
      supabase.from("workspaces").select("name").eq("id", diagnosis.workspace_id).single(),
      supabase.from("students").select("display_name,class_id").eq("id", diagnosis.student_id).single(),
      diagnosis.assessment_id
        ? supabase.from("assessments").select("title").eq("id", diagnosis.assessment_id).maybeSingle()
        : Promise.resolve({ data: null, error: null }),
    ]);
    if (workspaceResult.error || studentResult.error || !workspaceResult.data || !studentResult.data) {
      throw workspaceResult.error ?? studentResult.error ?? new Error("Report context is unavailable.");
    }
    ...
    const message = caught instanceof Error ? caught.message : "Parent report could not be generated.";
