...
import {
  ...
  ASSESSMENT_KINDS,
  ASSESSMENT_OVERALL_DIFFICULTIES,
  ASSESSMENT_PROMPT_VERSION_V11,
  buildWorldClassAssessmentPrompt,
  buildWorldClassAssessmentRepairPrompt,
  buildWorldClassAssessmentSystemInstruction,
  validateWorldClassAssessment,
  worldClassQualitySummary,
  type AssessmentKind,
  type AssessmentOverallDifficulty,
  type AssessmentTopicSpec,
  type WorldClassAssessmentRequest,
...
function parseObjectives(value: unknown, fallback = "") {
...
function parseTopics(
  ...
  fallbackObjective: unknown,
): AssessmentTopicSpec[] {
  let topics: AssessmentTopicSpec[] = [];
  if (Array.isArray(value)) {
    topics = value.map((entry, index) => {
      const row = record(entry);
      if (!row) throw new Error(`Topic ${index + 1} is invalid.`);
      const topic = requireString(row.topic, `Topic ${index + 1}`);
      const objectives = parseObjectives(row.objectives);
      const weight = Number(row.weight);
      if (!Number.isFinite(weight) || weight <= 0 || weight > 100) {
        throw new Error(`Topic ${index + 1} weight must be between 1 and 100.`);
      }
      return { topic, objectives, weight };
    });
  }

  if (!topics.length) {
    const topic = requireString(fallbackTopic, "Topic");
    const objective = requireString(fallbackObjective, "Objective");
    topics = [{ topic, objectives: [objective], weight: 100 }];
  }

  if (topics.length > MAX_TOPICS) {
    throw new Error(`Use no more than ${MAX_TOPICS} topics in one assessment.`);
  }

  const normalized = topics.map((entry) => entry.topic.toLowerCase().trim());
  if (new Set(normalized).size !== normalized.length) {
    throw new Error("Each assessment topic must be unique.");
  }

  const weightTotal = topics.reduce((sum, entry) => sum + entry.weight, 0);
  if (Math.abs(weightTotal - 100) > 0.01) {
    throw new Error(`Topic weights must add up to 100%. Current total: ${weightTotal}%.`);
  }

  return topics;
}

function validateGenerateInput(value: unknown): WorldClassAssessmentRequest {
  ...
  if (
    ...
    throw new Error("Select a valid question format.");
  }

  const assessmentKind = requireString(
    input.assessmentKind ?? "test",
    "Assessment type",
  ) as AssessmentKind;
  if (!ASSESSMENT_KINDS.includes(assessmentKind)) {
    throw new Error("Select a valid assessment type.");
  }

  const overallDifficulty = requireString(
    input.overallDifficulty ?? "medium",
    "Overall difficulty",
  ) as AssessmentOverallDifficulty;
  if (!ASSESSMENT_OVERALL_DIFFICULTIES.includes(overallDifficulty)) {
  ...
  return {
    ...
    topic: combinedTopic,
    objective: combinedObjective || "Assess the supplied topics appropriately.",
    assessmentMode,
    assessmentKind,
    overallDifficulty,
    topics,
  ...
  for (const row of orderedRows) {
    ...
    if (SUPPORTED_INLINE_MIME.has(row.mime_type)) {
      if (inlineBytes + blob.size > MAX_INLINE_RESOURCE_BYTES) {
        warnings.push(
          `${row.title}: skipped because selected sources exceed the safe inline limit.`,
        );
        continue;
      }
    ...
    warnings.push(
      `${row.title}: this file type cannot be read directly by the generator.`,
    );
  }
...
async function handleGenerate(
  ...
  try {
    ...
    runId = await startAiRun(supabase, {
      ...
      engineVersion: ASSESSMENT_ENGINE_VERSION_V11,
      promptVersion: ASSESSMENT_PROMPT_VERSION_V11,
      provider: "openai",
    ...
    const generatedResult = await generateOpenAIJson<unknown>({
      systemInstruction: buildWorldClassAssessmentSystemInstruction(),
      parts: [
        {
          text: buildWorldClassAssessmentPrompt(
            input,
      ...
      schemaName: "ksi_assessment_v11",
      maxOutputTokens: 12000,
    });

    let assessment = parseGeneratedAssessment(generatedResult.data);
    let validation = validateWorldClassAssessment(assessment, input);
    if (!validation.passed) {
      const repairedResult = await generateOpenAIJson<unknown>({
        systemInstruction: buildWorldClassAssessmentSystemInstruction(),
        parts: [
          {
            text: buildWorldClassAssessmentRepairPrompt(
              input,
              assessment,
              validation,
            ),
        ...
        responseSchema: ASSESSMENT_JSON_SCHEMA,
        schemaName: "ksi_assessment_v11_repair",
        maxOutputTokens: 12000,
      });
      assessment = parseGeneratedAssessment(repairedResult.data);
      validation = validateWorldClassAssessment(assessment, input);
    }
    ...
    if (!validation.passed) {
      ...
      return json(
        {
          error:
            "The generated assessment did not pass KAEC world-class assessment validation after repair, so it was not saved.",
    ...
    const qualitySummary = worldClassQualitySummary(assessment, input);
...
function topicsFromBlueprint(
  blueprint: Record<string, unknown>,
  edited: GeneratedAssessment,
): AssessmentTopicSpec[] {
  try {
    return parseTopics(
      blueprint.requestedTopics,
      blueprint.topic ?? edited.items[0]?.topic,
      blueprint.objective ?? edited.items[0]?.objective,
    );
  } catch {
    const first = edited.items[0];
    return [
      {
        topic: first?.topic || "General topic",
        objectives: first?.objective ? [first.objective] : [],
        weight: 100,
      },
    ];
  }
...
async function handleSaveEdits(
  ...
  const topics = topicsFromBlueprint(blueprint, edited);
  ...
  const validation = validateWorldClassAssessment(edited, request);
  if (!validation.passed) {
    return json(
      {
        error:
          "The edited assessment needs correction before it can be saved as validated.",
  ...
  const qualitySummary = worldClassQualitySummary(edited, request);
  const { error: assessmentError } = await supabase
    ...
    .update({
      ...
      engine_version: ASSESSMENT_ENGINE_VERSION_V11,
      prompt_version: ASSESSMENT_PROMPT_VERSION_V11,
    })
    .eq("id", assessmentId);
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "save_edits") {
      return await handleSaveEdits(supabase, body);
    }
