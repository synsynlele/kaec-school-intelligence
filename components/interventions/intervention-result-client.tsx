...
function parseActions(value: unknown): InterventionAction[] {
  ...
  return value
    .map((item) => {
      ...
      return {
        domain:
          row.domain === "skill" || row.domain === "character"
            ? row.domain
            : "academic",
...
export function InterventionResultClient({
  interventionId,
}: {
  ...
  const load = useCallback(async () => {
    ...
    if (!profile.default_workspace_id) {
      throw new Error("Choose an active workspace before opening this intervention.");
    }
    ...
    const firstError =
      workspaceResult.error ?? membershipResult.error ?? handoffResult.error;
    if (firstError) throw firstError;
    if (!workspaceResult.data || !handoffResult.data) {
      throw new Error("This intervention is unavailable in the active workspace.");
    }
    ...
    const nextState: State = {
      ...
      className,
      diagnosisSummary:
        diagnosisResult.data?.concise_diagnosis || "Approved diagnosis source",
      handoff,
    };
    setState(nextState);
          ...
          if (active) {
            setError(
              caught instanceof Error
                ? caught.message
                : "The intervention could not be opened.",
            );
          }
        })
        .finally(() => {
  ...
  async function save(confirm = false) {
    ...
    try {
      ...
      setNotice(
        confirm
          ? "Intervention confirmed and locked. It is ready to guide the next HQLS lesson."
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : "The intervention could not be saved.",
      );
    } finally {
      setBusy(null);
    }
  ...
  async function manage(action: "archive" | "delete") {
    ...
    if (action === "archive") {
      const confirmed = window.confirm(
        "Archive this intervention? It will leave active work but remain available as history.",
      );
      if (!confirmed) return;
    }
    ...
    if (action === "delete") {
      confirmation =
        window.prompt("Permanent deletion cannot be undone. Type DELETE to continue.") ??
        undefined;
    ...
    try {
      const response = await authenticatedFetch("/api/interventions/manage", {
        ...
        body: JSON.stringify({
          interventionId: state.handoff.id,
          action,
          confirmation,
        }),
      });
      ...
      if (!response.ok) {
        throw new Error(
          result.error || "The intervention management action could not be completed.",
        );
      }
      if (action === "delete") {
      ...
      await load();
      setNotice(
        "Intervention archived. Its diagnosis and linked-lesson provenance remain intact.",
      );
    } catch (caught) {
      setError(
        caught instanceof Error
          ? caught.message
          : "The intervention management action could not be completed.",
      );
    } finally {
      setBusy(null);
    }
  ...
  if (loading) {
    return (
      <div className="flex min-h-[55vh] items-center justify-center px-5 text-sm text-zinc-500">
    ...
    return (
      <main className="mx-auto max-w-3xl px-5 py-10">
        <div className="rounded-2xl border border-red-200 bg-red-50 p-5 text-sm text-red-800">
          {error ?? "Intervention unavailable."}
        </div>
      </main>
    );
  }
    ...
    <main className="mx-auto max-w-5xl px-4 py-6 sm:px-8 sm:py-10">
      ...
      <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-8">
        <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
          <div>
            <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold uppercase text-emerald-900">
              {state.handoff.status}
            </span>
            <p className="mt-4 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-800">
              Action & Intervention Result
            </p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight text-zinc-950 sm:text-3xl">
              {state.studentName}
            </h1>
            <p className="mt-2 text-sm text-zinc-600">
              {state.className} · {state.workspaceName}
            </p>
          </div>
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap md:justify-end">
            <Link
              href={`/diagnosis/result?diagnosis=${encodeURIComponent(state.handoff.diagnosis_id)}`}
              className="min-h-11 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-center text-sm font-semibold text-blue-900"
            >
              Open Source Diagnosis
            </Link>
            {state.handoff.next_lesson_id ? (
              <Link
                href={`/hqls/result?lesson=${encodeURIComponent(state.handoff.next_lesson_id)}`}
                className="min-h-11 rounded-xl bg-emerald-950 px-4 py-2.5 text-center text-sm font-semibold text-white"
              >
        ...
        <div className="mt-6 rounded-2xl border border-[#ddd4b7] bg-[#f8f4e8] p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-emerald-950">
            Approved diagnosis baseline
          </p>
      ...
      <section className="mt-5 rounded-3xl border border-emerald-950/10 bg-white p-5 shadow-sm sm:p-7">
        <TextArea
          label="Priority Growth Target"
          readOnly={readOnly}
          value={editor.priorityGrowthTarget}
          onChange={(value) => setEditor({ ...editor, priorityGrowthTarget: value })}
        />
        <TextArea
          label="Evidence Basis"
          readOnly={readOnly}
          value={editor.evidenceBasis}
          onChange={(value) => setEditor({ ...editor, evidenceBasis: value })}
        />
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <ActionList
            title="School Intervention"
            items={editor.schoolIntervention}
            readOnly={readOnly}
            onChange={(index, patch) =>
              updateAction("schoolIntervention", index, patch)
            }
          />
          <ActionList
            title="Parent Intervention"
            items={editor.parentIntervention}
            readOnly={readOnly}
            onChange={(index, patch) =>
              updateAction("parentIntervention", index, patch)
            }
          />
        </div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <Field label="Overall Timeframe">
            <input
              ...
              onChange={(event) =>
                setEditor({ ...editor, timeframe: event.target.value })
              }
              className="min-h-11 rounded-xl border border-zinc-300 bg-white px-3 text-sm read-only:bg-stone-50"
            />
          </Field>
          <Field label="Review Date / Checkpoint">
            <input
              ...
              onChange={(event) =>
                setEditor({ ...editor, reviewDate: event.target.value })
              }
              className="min-h-11 rounded-xl border border-zinc-300 bg-white px-3 text-sm read-only:bg-stone-50"
            />
          </Field>
        ...
        <TextArea
          ...
          readOnly={readOnly}
          value={editor.successIndicator}
          onChange={(value) => setEditor({ ...editor, successIndicator: value })}
        />
        <TextArea
          label="Next Learning Adjustment"
          readOnly={readOnly}
          value={editor.nextLearningAdjustment}
          onChange={(value) =>
            setEditor({ ...editor, nextLearningAdjustment: value })
          }
        />
      </section>
      ...
      <section className="mt-5 rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap gap-2">
          {state.handoff.status === "draft" ? (
            <>
              <button
                type="button"
                disabled={busy !== null}
                onClick={() => void save(false)}
                className="rounded-xl border border-zinc-300 px-4 py-2.5 text-sm font-semibold disabled:opacity-50"
              >
          ...
          {state.canManage && state.handoff.status !== "archived" ? (
            <button
              type="button"
              disabled={busy !== null}
            ...
            >
              {busy === "archive" ? "Archiving…" : "Archive Intervention"}
            </button>
          ) : null}
          {state.canManage && state.handoff.status === "archived" ? (
            <button
              ...
              className="rounded-xl border border-red-300 px-4 py-2.5 text-sm font-semibold text-red-700 disabled:opacity-50"
            >
              {busy === "delete" ? "Deleting…" : "Permanent Delete"}
            </button>
          ) : null}
        </div>
        <p className="mt-3 text-xs leading-5 text-zinc-500">
          Confirmed: {dateLabel(state.handoff.confirmed_at)}. Archived plans are
          immutable. If a confirmed intervention has already produced a linked HQLS
          lesson, permanent deletion remains blocked so the lesson keeps its provenance.
        </p>
      </section>
  ...
  return (
    <label className="mt-5 grid gap-2 text-sm font-semibold text-zinc-800">
      ...
      <textarea
        ...
        className="min-h-28 rounded-xl border border-zinc-300 bg-white p-3 text-sm font-normal leading-6 read-only:bg-stone-50"
      />
    </label>
  );
}
...
function ActionList({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-emerald-950/15">
      <div className="bg-emerald-950 px-4 py-2.5 text-center text-sm font-semibold text-white">
        {title}
      </div>
        ...
        {items.length ? (
          items.map((item, index) => (
            <div
              ...
              className="rounded-xl border border-zinc-200 bg-stone-50 p-3"
            >
              ...
              <textarea
                ...
                className="mt-2 min-h-24 w-full rounded-lg border border-zinc-300 bg-white p-2 text-sm leading-6 read-only:bg-stone-50"
              />
              <input
                readOnly={readOnly}
                value={item.timeframe}
                onChange={(event) =>
                  onChange(index, { timeframe: event.target.value })
                }
                className="mt-2 w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm read-only:bg-stone-50"
              />
            </div>
          ))
        ) : (
          <p className="text-sm text-zinc-500">No action recorded.</p>
        )}
      </div>
    </div>
  );
