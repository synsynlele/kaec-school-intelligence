...
async function loadContext(supabase: SupabaseClient): Promise<Context | null> {
  ...
  return { workspaceName: workspaceResult.data.name, role: membershipResult.data.role, scheme: schemeResult.data as Context["scheme"], queue: queueResult.data as Context["queue"] };
}
...
export function SchemeIngestionClient() {
  ...
  return <main className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
    ...
    <section className="mt-5 rounded-3xl bg-emerald-950 p-7 text-white shadow-sm sm:p-9"><p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-200">Stage 12 · Lagos sequencing layer</p><h1 className="mt-2 text-3xl font-bold">Scheme Ingestion</h1><p className="mt-3 max-w-4xl text-sm leading-6 text-emerald-50/90">Extract weekly scheme rows, verify them against the supplied source, then deliberately promote approved rows into the versioned Lagos sequence. Approval never turns a scheme row into an official NERDC curriculum objective.</p></section>
