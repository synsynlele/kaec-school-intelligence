-- KSI 2.0 Stage 8 — secure student account onboarding.
-- School admins issue one-time codes; authenticated learners redeem them.

create table public.student_access_invites (
  ...
  constraint student_access_invites_student_workspace_fk
    foreign key (student_id, workspace_id)
    references public.students(id, workspace_id)
    on delete cascade
);

create index student_access_invites_workspace_student_idx
  on public.student_access_invites(workspace_id, student_id, created_at desc);
create index student_access_invites_expires_idx
  on public.student_access_invites(expires_at)
  where redeemed_at is null and revoked_at is null;

alter table public.student_access_invites enable row level security;
revoke all on public.student_access_invites from anon, authenticated;
...
begin
  ...
  update public.student_access_invites
  set revoked_at = now()
...
begin
  ...
  from public.student_access_invites
  ...
  where id = invite_row.workspace_id;
  ...
  where id = invite_row.student_id
    and workspace_id = invite_row.workspace_id
    and active = true;
  ...
  insert into public.student_accounts(
    user_id, student_id, workspace_id, active, created_by
  ) values (
    auth.uid(), student_row.id, invite_row.workspace_id, true, invite_row.issued_by
  )
  on conflict (user_id)
  do update set
...
comment on table public.student_access_invites is
  'One-time hashed access codes issued by school admins to link authenticated learners to existing KSI student records.';
comment on function public.issue_student_access_code(uuid,integer) is
  'Admin-only issuance of one-time Student KSI access codes. Raw codes are returned once and never stored.';
comment on function public.redeem_student_access_code(text) is
  'Authenticated learner redemption flow that creates student membership and one-to-one student account binding.';